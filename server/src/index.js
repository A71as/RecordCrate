...
// Always mount routes that don't require DB
app.use("/api/discography", discographyRouter);
app.use("/api/search", nlsearchRouter);
...
async function start() {
  if (!MONGODB_URI) {
    console.warn(
      "[recordcrate-api] No MONGODB_URI provided. Starting in discography-only mode."
    );
    app.listen(PORT, () =>
      console.log(
        `API listening (discography-only) on http://localhost:${PORT}`
      )
    );
    return;
  }
